...
int main()
{
    ...
    for(int i = 0 ; i < strlen(s) ;i++){
        ...
        int lastUpper = isupper(s[i-1]);
        int currentUpper = isupper(s[i]);
        if ( currentUpper != lastUpper  || i == strlen(s)-1)
        {
            if(currentContinue == k ){
                _continue += k;
                maxContinue = _continue > maxContinue ? _continue : maxContinue;
            }else if(currentContinue > k){
                _continue += k;
                maxContinue = _continue > maxContinue ? _continue : maxContinue;
                _continue = k ;
            }
